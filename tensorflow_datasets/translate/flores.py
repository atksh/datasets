...
class FloresConfig(tfds.core.BuilderConfig):
    ...
    def __init__(self, text_encoder_config=None, language_pair=(None, None), **kwargs):
        """BuilderConfig for FLoRes.
    ...
    """
        encoder_name = text_encoder_config.name if text_encoder_config else "plain_text"
        name = "%s%s_%s" % (language_pair[0], language_pair[1], encoder_name)

        description = ("Translation dataset from %s to %s, uses encoder %s.") % (
            language_pair[0],
            language_pair[1],
            encoder_name,
        )
    ...
    def _split_generators(self, dl_manager):
        ...
        return [
            tfds.core.SplitGenerator(
                name=tfds.Split.VALIDATION, num_shards=1, gen_kwargs=files["dev"]
            ),
            tfds.core.SplitGenerator(
                name=tfds.Split.TEST, num_shards=1, gen_kwargs=files["devtest"]
            ),
        ]
