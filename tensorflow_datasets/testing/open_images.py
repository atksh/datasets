...
def _write_image_level_labels(fname, image_ids, machine=False):
    ...
    all_class_label = ClassLabel(
        names_file=py_utils.get_tfds_path(
...
def _generate_train_files():
    ...
    _write_image_level_labels("train-machine-labels.csv", all_image_ids, machine=True)
...
def _generate_validation_files():
    ...
    _write_image_level_labels("validation-machine-labels.csv", image_ids, machine=True)
