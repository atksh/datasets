...
def _yield_mammography_with_abnormalities():
    ...
    mammography = np.zeros((MAMMOGRAPHY_HEIGHT, MAMMOGRAPHY_WIDTH), dtype=np.uint8)
    ...
    breast_intensity = np.random.randint(BREAST_INTENSITY_MIN, BREAST_INTENSITY_MAX + 1)
    mammography[
...
def _yield_csv_rows_base(output_dir, row_extra_info_gen_fn):
    ...
    write_png(os.path.join(output_dir, mammography_basename + ".png"), mammography)
