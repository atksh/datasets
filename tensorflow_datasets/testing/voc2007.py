...
def _generate_jpeg(example_id, height, width):
    ...
    filepath = os.path.join(
        _voc2007_output_dir(), "VOCdevkit/VOC2007/JPEGImages/%06d.jpg" % example_id
    )
    dirname = os.path.dirname(filepath)
    if not tf.io.gfile.exists(dirname):
        tf.io.gfile.makedirs(dirname)
...
def _generate_annotation(example_id, height, width):
    ...
    filepath = os.path.join(
        _voc2007_output_dir(), "VOCdevkit/VOC2007/Annotations/%06d.xml" % example_id
    )
...
def _generate_data_for_set(set_name, example_start, num_examples):
    ...
    filepath = os.path.join(
        _voc2007_output_dir(), "VOCdevkit/VOC2007/ImageSets/Main/%s.txt" % set_name
    )
    _write_text_file(
        filepath,
        "".join(
