...
def _get_mapping_files():
    ...
    fobj_rand = tempfile.NamedTemporaryFile(delete=False, mode="wb", suffix=".txt")
...
def _create_zip_files():
    ...
    with zipfile.ZipFile(label_out_path, "w") as label_zip:
        for i in range(NUM_IMAGES):
            ...
            label_zip.write(
                label, os.path.join("training", "label_2", "label_{:06d}.txt".format(i))
            )
    ...
    with zipfile.ZipFile(devkit_out_path, "w") as devkit_zip:
        ...
        devkit_zip.write(train_mapping, os.path.join("mapping", "train_mapping.txt"))
