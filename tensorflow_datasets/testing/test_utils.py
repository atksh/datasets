        ...
        if test.raise_cls is not None:
            ...
            if test.expected_serialized is not None:
                with self._subTest("out_serialize"):
                    self.assertEqual(
                        test.expected_serialized, feature.encode_example(test.value),
                    )

            # Test serialization + decoding from disk
            with self._subTest("out"):
                out_tensor, out_numpy = features_encode_decode(
                    fdict, input_value, decoders={"inner": test.decoders},
                )
                ...
                with self._subTest("shape"):
                    ...
                    utils.map_nested(
                        lambda x: x[0].shape.assert_is_compatible_with(x[1]), out_shapes
                    )
...
def features_encode_decode(features_dict, example, decoders):
    ...
    with tmp_dir() as tmp_dir_:
        ...
        file_adapter.write_from_generator(
            generator=[encoded_example], output_files=[tmp_filename],
        )
        ds = file_adapter.dataset_from_filename(tmp_filename)

        # Decode the example
        decode_fn = functools.partial(features_dict.decode_example, decoders=decoders,)
        ds = ds.map(decode_fn)
    ...
    def _split_generators(self, dl_manager):
        ...
        return [
            splits.SplitGenerator(
                name=splits.Split.TRAIN, gen_kwargs={"range_": range(20)}
            ),
            splits.SplitGenerator(
                name=splits.Split.TEST, gen_kwargs={"range_": range(20, 30)}
            ),
        ]
...
def mock_kaggle_api(filenames=None, err_msg=None):
  ...
  """
        ...
        def check_output(command_args):
            ...
            if err_msg:
                raise subprocess.CalledProcessError(
                    1, command_args, tf.compat.as_bytes(err_msg)
                )
    ...
    with absltest.mock.patch(
        "subprocess.check_output", make_mock_check_output(filenames, err_msg)
    ):
        yield
