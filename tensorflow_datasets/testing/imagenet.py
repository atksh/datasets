...
def _ilsvrc2012_output_dir():
    return os.path.join(
        FLAGS.tfds_dir, "testing", "test_data", "fake_examples", "imagenet2012"
    )


def _generate_train_archive():
    """Generate train archive."""
    output_path = os.path.join(_ilsvrc2012_output_dir(), "ILSVRC2012_img_train.tar")
    tar = tarfile.open(output_path, mode="w")
    names_file = tfds.core.get_tfds_path(
        imagenet._LABELS_FNAME
