...
class GrooveConfig(tfds.core.BuilderConfig):
    ...
    def __init__(self, split_bars=None, include_audio=True, audio_rate=16000, **kwargs):
...
class Groove(tfds.core.GeneratorBasedBuilder):
    ...
    BUILDER_CONFIGS = [
        GrooveConfig(
            include_audio=False, description="Groove dataset without audio, unsplit."
        ),
        GrooveConfig(
...
def _load_wav(path, sample_rate):
    with tf.io.gfile.GFile(path, "rb") as audio_f:
        audio_segment = (
            tfds.core.lazy_imports.pydub.AudioSegment.from_file(audio_f, format="wav")
            .set_channels(1)
