...
class Nsynth(tfds.core.BeamBasedBuilder):
    ...
    def _info(self):
        ...
        if self.builder_config.estimate_f0_and_loudness:
            ...
            features["f0"] = {
                "hz": tfds.features.Tensor(shape=f0_and_ld_shape, dtype=tf.float32),
                "midi": tfds.features.Tensor(shape=f0_and_ld_shape, dtype=tf.float32),
                "confidence": tfds.features.Tensor(
                    shape=f0_and_ld_shape, dtype=tf.float32
                ),
            }
            features["loudness"] = {
    ...
    def _split_generators(self, dl_manager):
        ...
        dl_urls = {}
        dl_urls["examples"] = {
            split: _BASE_DOWNLOAD_PATH + "%s.tfrecord.tar" % split for split in _SPLITS
        }
    ...
    def _build_pcollection(self, pipeline, tfrecord_dirs, ids, split):
        ...
        def _compute_loudness(ex):
            ...
            mean_loudness_db = librosa.amplitude_to_db(
                mean_loudness_amp, amin=amin, top_db=top_db
            )
        ...
        if self.builder_config.estimate_f0_and_loudness:
            ...
            if split == tfds.Split.TRAIN:
                ...
                loudness_mean = (
                    loudness | "loudness_mean" >> beam.combiners.Mean.Globally()
                )
                loudness_variance = (
                    loudness
                    | beam.Map(
                        lambda ld, ld_mean: (ld - ld_mean) ** 2,
