...
class ExampleParser(object):
    ...
    """
        ...
        return {k: build_single_spec(k, v) for k, v in self._flat_example_specs.items()}
    ...
    """
        ...
        example = tf.io.parse_single_example(
            serialized=serialized_example, features=flat_feature_specs,
        )
...
def _to_tf_example_spec(tensor_info):
    ...
    if all(s is not None for s in tensor_info.shape):
        ...
        tf_specs = {
            "ragged_row_lengths_{}".format(
                k
            ): tf.io.FixedLenSequenceFeature(  # pylint: disable=g-complex-comprehension
