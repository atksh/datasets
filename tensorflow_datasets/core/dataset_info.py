  ...
  """
    ...
    """
        ...
        if urls:  # TODO(epot):Delete field once every user have been migrated
            raise ValueError(
                "`urls=` field is deprecated. Please use "
...
def get_dataset_feature_statistics(builder, split):
    ...
    for example in utils.tqdm(np_dataset, unit=" examples", leave=False):
        ...
        for feature_name in feature_names:
            ...
            is_numeric = (
                np.issubdtype(feature_dtype, np.number) or feature_dtype == np.bool_
            )
            if is_numeric:
...
def read_from_json(json_filename):
    ...
    parsed_proto = json_format.Parse(
        dataset_info_json_str, dataset_info_pb2.DatasetInfo()
    )
    return parsed_proto
    ...
    """
        ...
        if isinstance(item, beam.pvalue.PValue):
            ...
            _ = (
                ...
                >> beam.io.WriteToText(
                    self._temp_filepath(key), num_shards=1, shard_name_template=""
                )
            )
