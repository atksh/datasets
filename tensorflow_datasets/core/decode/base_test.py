...
class BaseDecodeTest(testing.FeatureExpectationsTestCase):
        ...
        class DecodeCrop(decode_lib.Decoder):
            ...
            def decode_example(self, serialized_image):
                return tf.image.decode_and_crop_jpeg(
                    serialized_image, [y, x, h, w], channels=self.feature.shape[-1],
                )

        @decode_lib.make_decoder()
        def decode_crop(serialized_image, feature):
            return tf.image.decode_and_crop_jpeg(
                serialized_image, [y, x, h, w], channels=feature.shape[-1],
            )
    ...
    def test_video_custom_decode(self):
        ...
        self.assertFeature(
            ...
            shape={"image": (30, 60, 3), "label": (),},
            dtype={"image": tf.uint8, "label": tf.int64,},
            tests=[
                testing.FeatureExpectationItem(
                    decoders={"image": decode_lib.SkipDecoding(),},
                    value={"image": image_path, "label": 123,},
                    expected={"image": serialized_img, "label": 123,},
                    shape={"image": (), "label": (),},
                    dtype={"image": tf.string, "label": tf.int64,},
                ),
            ],
        )
