    ...
    def _split_generators(self, dl_manager):
        ...
        return [
            splits_lib.SplitGenerator(
                name=splits_lib.Split.TRAIN, gen_kwargs={"range_": range(20)},
            ),
            splits_lib.SplitGenerator(
                name=splits_lib.Split.TEST, gen_kwargs={"range_": range(20, 30)},
            ),
        ]
...
class DatasetBuilderTest(testing.TestCase):
    ...
    def test_config_construction(self):
        with testing.tmp_dir(self.get_temp_dir()) as tmp_dir:
            ...
            self.assertIs(plus1_config, builder.builder_config)
            self.assertIs(
                builder.builder_config, DummyDatasetWithConfigs.BUILDER_CONFIGS[0]
            )
    ...
    def test_invalid_split_dataset(self):
        with testing.tmp_dir(self.get_temp_dir()) as tmp_dir:
            with self.assertRaisesWithPredicateMatch(ValueError, "ALL is a special"):
                ...
                registered.load(
                    name="invalid_split_dataset", data_dir=tmp_dir,
                )
    ...
    def setUp(self):
        ...
        def load_mnist_dataset_info(self):
            mnist_info_path = os.path.join(
                utils.tfds_dir(), "testing/test_data/dataset_info/mnist/1.0.0",
            )
...
class NestedSequenceBuilder(dataset_builder.GeneratorBasedBuilder):
    ...
    def _split_generators(self, dl_manager):
        ...
        del dl_manager
        return [
            splits_lib.SplitGenerator(name=splits_lib.Split.TRAIN, gen_kwargs={},),
        ]
    ...
    def test_nested_sequence(self):
        with testing.tmp_dir(self.get_temp_dir()) as tmp_dir:
            ...
            self.assertAllEqual(
                ex2,
                tf.ragged.constant(
                    [[[10, 11]], [[12, 13], [14, 15]],], inner_shape=(2,)
                ),
            )

            self.assertEqual(
                ds_info.features.dtype, {"frames": {"coordinates": tf.int32}},
            )
            self.assertEqual(
                ds_info.features.shape, {"frames": {"coordinates": (None, None, 2)}},
            )
            nested_tensor_info = ds_info.features.get_tensor_info()
            self.assertEqual(
                nested_tensor_info["frames"]["coordinates"].sequence_rank, 2,
            )
