...
class PyUtilsTest(testing.TestCase):
    ...
    def test_map_nested(self):
        ...
        result = py_utils.map_nested(map_fn, {"a": 1, "b": {"c": 2, "e": [3, 4, 5],},})
        self.assertEqual(result, {"a": 10, "b": {"c": 20, "e": [30, 40, 50],},})
    ...
    def test_zip_nested(self):
        ...
        self.assertEqual(
            result,
            {"a": (1, 10), "b": {"c": (2, 20), "e": [(3, 30), (4, 40), (5, 50)],},},
        )
    ...
    def test_flatten_nest_dict(self):
        ...
        with self.assertRaisesWithPredicateMatch(ValueError, "Extra keys"):
            py_utils.pack_as_nest_dict(
                {"a": 1, "b/c": 2, "b/e": 3, "b/f/g": 4, "b/h": 5,}, nest_d  # Extra key
            )

        with self.assertRaisesWithPredicateMatch(KeyError, "b/e"):
            py_utils.pack_as_nest_dict(
                {"a": 1, "b/c": 2, "b/d": 3,},
                {"a": 1, "b": {"c": 2, "d": 3, "e": 4,}},  # Extra key
            )

        with self.assertRaisesWithPredicateMatch(ValueError, "overwrite existing key:"):
            py_utils.flatten_nest_dict(
                {"a": {"b": 1,}, "a/b": 2,}  # Collision
            )
...
class ReadChecksumDigestTest(testing.TestCase):
    def test_digest(self):
        ...
        self.assertEqual(
            digest, "04f38ebed34d3b027d2683193766155912fba647158c583c3bdb4597ad8af34c"
        )
