    ...
    """
        ...
        if not (k or percent or weighted):
            raise ValueError(
                ...
                "One of k, weighted or percent should be set to a non empty value.".format(
                    arg
                )
            )
  ...
  """
    ...
    def __eq__(self, other):
        ...
        if isinstance(other, NamedSplit):
            ...
            raise ValueError(
                "Equality not supported between split {} and {}".format(self, other)
            )
...
SlicedSplitInfo = collections.namedtuple(
    "SlicedSplitInfo", ["split_info", "slice_value",]
)
...
class SplitDict(utils.NonMutableDict):
    ...
    def __getitem__(self, key):
        if str(key) not in self:
            raise KeyError(
                "Invalid split %s. Available splits are: %s"
...
def check_splits_equals(splits1, splits2):
    ...
    for _, (split1, split2) in utils.zip_dict(splits1, splits2):
        if (
            split1.num_shards != split2.num_shards
