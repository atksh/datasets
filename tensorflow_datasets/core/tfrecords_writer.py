...
class _TFRecordWriter(object):
    ...
    """
        ...
        self._shard_boundaries = [
            round(num_examples * (float(i) / number_of_shards))
...
class Writer(object):
  ...
  """
    ...
    def finalize(self):
        ...
        number_of_shards = _get_number_shards(self._shuffler.size, self._num_examples)
        writer = _TFRecordWriter(self._path, self._num_examples, number_of_shards)
        for serialized_example in utils.tqdm(
            self._shuffler, total=self._num_examples, unit=" examples", leave=False
        ):
            writer.write(serialized_example)
        shard_lengths = writer.finalize()
        logging.info("Done writing %s. Shard lengths: %s", self._path, shard_lengths)
