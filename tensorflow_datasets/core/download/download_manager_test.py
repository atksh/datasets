...
class DownloadManagerTest(testing.TestCase):
    ...
    def test_download(self):
        ...
        _ = [
            self._add_file(path, content)
            for path, content in [  # pylint: disable=g-complex-comprehension
        ...
        manager = self._get_manager(
            checksums=dict((art.url, art.size_checksum) for art in (a, b, c))
        )
        dl_b.set()
        dl_c.set()
    ...
    def test_extract_twice_parallel(self):
        ...
        # extraction once.
        extracted_new, self.extract_results["/dl_dir/foo.tar"] = _get_promise_on_event(
            "/extract_dir/TAR.foo"
        )
        manager = self._get_manager()
        extracted_new.set()
    ...
    def test_download_and_extract(self):
        ...
        dl_b, self.dl_results[b.url] = _get_promise_on_event(b.checksum_size)
        ext_a, self.extract_results["/dl_dir/%s" % a.dl_fname] = _get_promise_on_event(
            "/extract_dir/ZIP.%s" % a.dl_fname
        )
        ...
        manager = self._get_manager(
            checksums={a.url: a.size_checksum, b.url: b.size_checksum,}
        )
    ...
    def test_download_and_extract_archive_ext_in_fname(self):
        ...
        ext, self.extract_results["/dl_dir/%s" % a.dl_fname] = _get_promise_on_event(
            "/extract_dir/ZIP.%s" % a.dl_fname
        )
        dl.set()
        ext.set()
        manager = self._get_manager(checksums={a.url: a.size_checksum,})
    ...
    def test_download_and_extract_already_downloaded(self):
        ...
        self._write_info("/dl_dir/%s.INFO" % a.dl_fname, {"original_fname": "a.zip"})
        ext_a, self.extract_results["/dl_dir/%s" % a.dl_fname] = _get_promise_on_event(
            "/extract_dir/ZIP.%s" % a.dl_fname
        )
        ext_a.set()
    ...
    def test_force_download_and_extract(self):
        ...
        self._write_info("/dl_dir/%s.INFO" % a.dl_fname, {"original_fname": "b.tar.gz"})
        dl_a, self.dl_results[a.url] = _get_promise_on_event(a.checksum_size)
        ext_a, self.extract_results["/dl_dir/%s" % a.dl_fname] = _get_promise_on_event(
            "/extract_dir/TAR_GZ.%s" % a.dl_fname
        )
        ...
        manager = self._get_manager(
            ...
            checksums={a.url: a.size_checksum,},
        )
        res = manager.download_and_extract(a.url)
        ...
        self.assertTrue(
            re.match(
                r"/dl_dir/%s\.tmp\.[a-h0-9]{32}/b.tar.gz" % a.dl_tmp_dirname, from_
            )
        )
    ...
    def test_wrong_checksum(self):
        ...
        dl_a, self.dl_results[a.url] = _get_promise_on_event(a.checksum_size)
        dl_a.set()
        manager = self._get_manager(checksums={a.url: (a.size, sha_b),})
