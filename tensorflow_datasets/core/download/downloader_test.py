...
class DownloaderTest(testing.TestCase):
    def setUp(self):
        ...
        absltest.mock.patch.object(
            downloader.ssl, "_create_unverified_context", lambda *a, **kw: "skip_ssl"
        ).start()
        absltest.mock.patch.object(
            downloader.ssl, "create_default_context", lambda *a, **kw: "use_ssl"
        ).start()
    ...
    def test_ftp_error(self):
        ...
        absltest.mock.patch.object(
            downloader.urllib.request, "urlopen", side_effect=error,
        ).start()
    ...
    def test_ssl_mock(self):
        ...
        absltest.mock.patch.dict(
            os.environ, {"TFDS_CA_BUNDLE": "/path/to/dummy.pem"}
        ).start()
        self.test_ftp_ssl("use_ssl")

    def test_ftp_ssl(self, ssl_type="skip_ssl"):
        absltest.mock.patch.object(
            downloader.urllib.request, "Request", lambda *a, **kw: "dummy_request"
        ).start()
    ...
    def test_py2_ftp_ssl_mock(self):
        ...
        absltest.mock.patch.dict(
            downloader.ssl.__dict__, ssl_mock_dict, clear=True
        ).start()
        method = absltest.mock.patch.object(
            downloader.logging, "info", return_value=None
        ).start()
        self.test_ftp()
...
class GetFilenameTest(testing.TestCase):
    ...
    def test_headers(self):
        cdisp = 'attachment;filename="hello.zip";' "filename*=UTF-8''hello.zip"
        resp = _FakeResponse(
