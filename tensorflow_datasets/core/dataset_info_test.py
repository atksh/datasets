...
_INFO_DIR = os.path.join(
    _TFDS_DIR, "testing", "test_data", "dataset_info", "mnist", "1.0.0"
)
_INFO_DIR_UNLABELED = os.path.join(
    _TFDS_DIR, "testing", "test_data", "dataset_info", "mnist_unlabeled", "1.0.0"
)
...
class DatasetInfoTest(testing.TestCase):
    ...
    def test_undefined_dir(self):
        with self.assertRaisesWithPredicateMatch(
            ValueError, "undefined dataset_info_dir"
        ):
    ...
    def test_writing(self):
        # First read in stuff.
        mnist_builder = mnist.MNIST(data_dir=tempfile.mkdtemp(dir=self.get_temp_dir()))
    ...
    def test_restore_after_modification(self):
        ...
        with testing.tmp_dir(self.get_temp_dir()) as tmp_dir:
            ...
            self.assertEqual(
                restored_info.download_checksums,
                {"url2": "some other checksum (new)", "url3": "some checksum (new)",},
            )
