...
class GetDatasetFilesTest(testing.TestCase):
    ...
    def test_skip_take1(self):
        ...
        files = self._get_files(instruction)
        self.assertEqual(
            files, [{"skip": 1, "take": 1, "filename": self.PATH_PATTERN % 0},]
        )
...
class ReadInstructionTest(testing.TestCase):
    ...
    def test_valid(self):
        ...
        self.check_from_spec(
            "train+test", [("train", None, None), ("test", None, None),]
        )
        # Absolute slicing:
    ...
    def test_add_invalid(self):
        ...
        with self.assertRaisesWithPredicateMatch(AssertionError, "different rounding"):
            unused_ = ri1 + ri2
    ...
    def test_invalid_spec(self):
        ...
        # Unexisting split:
        self.assertRaises("imaginary", 'Requested split "imaginary" does not exist')
        # Invalid boundaries abs:
        self.assertRaises("validation[:31]", "incompatible with 30 examples")
        # Invalid boundaries %:
        self.assertRaises(
            "validation[:250%]", "Percent slice boundaries must be > -100 and < 100"
        )
        self.assertRaises(
            "validation[-101%:]", "Percent slice boundaries must be > -100 and < 100"
        )
...
class ReaderTest(testing.TestCase):
    ...
    def test_nodata_instruction(self):
        ...
        with self.assertRaisesWithPredicateMatch(
            AssertionError, "corresponds to no data!"
        ):
    ...
    def test_shuffle_files(self):
        self._write_tfrecord("train", 5, "abcdefghijkl")
        ds = self.reader.read("mnist", "train", self.SPLIT_INFOS, shuffle_files=True)
