...
class TextEncoderConfig(object):
    ...
    def __init__(self, encoder=None, encoder_cls=None, vocab_size=None, name=None):
        if encoder:
...
class ByteTextEncoder(TextEncoder):
    ...
    """
        self._additional_tokens, self._additional_tokens_re = _prepare_reserved_tokens(
            additional_tokens
        )
...
class TokenTextEncoder(TextEncoder):
  ...
  """
    ...
    """
        ...
        # Note that internally everything is 0-indexed. Padding is dealt with at the
        # end of encode and the beginning of decode.
        self._token_to_id = dict(zip(self._vocab_list, range(len(self._vocab_list))))
