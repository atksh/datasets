  ...
  """
    ...
    def _token_to_subwords(self, token):
        ...
        while start < len(token):
            subword = None
            for end in range(min(len(token), start + self._max_subword_len), start, -1):
    ...
    def _build_from_token_counts(
        ...
        for _ in range(num_iterations):
            ...
            for token, count in six.iteritems(token_counts):
                start_idx = 0
                for subword in encoder._token_to_subwords(
                    token
...
def _validate_build_arguments(max_subword_length, reserved_tokens, target_vocab_size):
