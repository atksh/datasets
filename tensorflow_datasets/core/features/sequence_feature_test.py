...
class SequenceDictFeatureTest(testing.FeatureExpectationsTestCase):
        ...
        self.assertFeature(
            ...
            tests=[
                ...
                testing.FeatureExpectationItem(
                    value={"int": [1, 2, 3]}, expected={"int": [1, 2, 3]},
                ),
                # Numpy array
                testing.FeatureExpectationItem(
    ...
    def test_label(self):
        ...
        self.assertFeature(
            feature=feature_lib.Sequence(
                {"label": feature_lib.ClassLabel(names=["left", "right"]),}, length=None
            ),
            shape={"label": (None,)},
    ...
    def test_nested(self):
        ...
        self.assertFeature(
            ...
            shape={"a": (None,), "b": {"c": (None, 4, 2), "d": (None,),}},
            dtype={"a": tf.string, "b": {"c": tf.int32, "d": tf.uint8,}},
            tests=[
                testing.FeatureExpectationItem(
                    value={
                        "a": ["aa", "b", "ccc"],
    ...
    def test_encoding(self):
        ...
        f = feature_lib.Sequence(
            {"a": feature_lib.Sequence({"c": tf.int64}), "b": tf.int64,}
        )
        ...
        ex3 = f.encode_example(
            {
                "a": [[{"c": 1}, {"c": 1}, {"c": 1}], [], [{"c": 3}, {"c": 3}],],
                "b": [1, 2, 3],
            }
        )

        ex4 = f.encode_example({"a": {"c": [[1, 1, 1], [], [3, 3]]}, "b": [1, 2, 3],})

        out = {
            "a": {"c": tf.ragged.constant([[1, 1, 1], [], [3, 3],])},
            "b": [1, 2, 3],
        }
        ...
        with self.assertRaisesWithPredicateMatch(ValueError, "length of all elements"):
            f.encode_example(
                {"a": {"c": [[1, 1, 1], []]}, "b": [1, 2, 3],}
            )
        ...
        self.assertAllEqualNested(
            ex2,
            {"a": {"c": np.zeros((0, 0), np.int64)}, "b": np.zeros((0,), np.int64),},
        )
    ...
    def test_2lvl_sequences_mixed(self):
        ...
        self.assertFeature(
            feature=feature_lib.Sequence(
                {"a": feature_lib.Sequence(tf.int32), "b": tf.int32,}
            ),
            shape={"a": (None, None), "b": (None,),},
            dtype={"a": tf.int32, "b": tf.int32,},
            tests=[
                testing.FeatureExpectationItem(
                    value={"a": [[1, 1, 1], [], [3, 3]], "b": [1, 2, 3],},
                    expected={"a": [[1, 1, 1], [], [3, 3]], "b": [1, 2, 3],},
                ),
            ],
        )
    ...
    def test_2lvl_sequences(self):
        ...
        self.assertFeature(
            feature=feature_lib.Sequence(
                feature_lib.Sequence(feature_lib.Tensor(shape=(2,), dtype=tf.int32),),
            ),
            shape=(None, None, 2),
            dtype=tf.int32,
            tests=[
                testing.FeatureExpectationItem(
                    ...
                    expected=testing.RaggedConstant(
                        [[[0, 1], [2, 3]], [], [[4, 5]],], inner_shape=(2,)
                    ),
                ),
                # Empty
                testing.FeatureExpectationItem(value=[], expected=[],),
                # List of empty lists
                testing.FeatureExpectationItem(
                    value=[[], [], []], expected=[[], [], []],
                ),
                ...
                testing.FeatureExpectationItem(
                    value=[np.ones(shape=(3, 2), dtype=np.float32),],
                    raise_cls=ValueError,
    ...
    def test_2lvl_sequences_string(self):
        ...
        self.assertFeature(
            ...
            tests=[
                testing.FeatureExpectationItem(
                    value=[["abcd", "", "efg"], [], ["", ""], ["hij"],],
                    expected=[[b"abcd", b"", b"efg"], [], [b"", b""], [b"hij"],],
                ),
                testing.FeatureExpectationItem(value=[[], [],], expected=[[], [],],),
                testing.FeatureExpectationItem(
                    value=[["abcd", "efg", 123],],
    ...
    def test_3lvl_sequence(self):
        ...
        self.assertFeature(
            feature=feature_lib.Sequence(
                feature_lib.Sequence(feature_lib.Sequence(tf.int32), length=3,),
            ),
            shape=(None, 3, None),
            dtype=tf.int32,
            tests=[
                testing.FeatureExpectationItem(
                    value=[[[1, 2, 3], [], [4, 5]], [[10, 11], [12, 13], [14]],],
                    expected=[[[1, 2, 3], [], [4, 5]], [[10, 11], [12, 13], [14]],],
                ),
                testing.FeatureExpectationItem(
                    value=[
    ...
    def test_image(self):
        ...
        self.assertFeature(
            feature=feature_lib.Sequence(
                {"image": feature_lib.Image(shape=(128, 100, 3)),}, length=None
            ),
            ...
            tests=[
                ...
                testing.FeatureExpectationItem(
                    value={"image": imgs_stacked}, expected={"image": imgs_stacked},
                ),
                testing.FeatureExpectationItem(
                    value={"image": imgs}, expected={"image": imgs_stacked},
                ),
                # Empty value
                testing.FeatureExpectationItem(
                    value={"image": []},
...
class SequenceFeatureTest(testing.FeatureExpectationsTestCase):
    def test_int(self):
        ...
        self.assertFeature(
            ...
            tests=[
                # Python array
                testing.FeatureExpectationItem(value=[1, 2, 3], expected=[1, 2, 3],),
                # Numpy array
                testing.FeatureExpectationItem(
                    value=np.ones(shape=(3,), dtype=np.int32), expected=[1, 1, 1],
                ),
                # Wrong sequence length
                testing.FeatureExpectationItem(
    ...
    def test_label(self):
        ...
        self.assertFeature(
            ...
            tests=[
                testing.FeatureExpectationItem(
                    value=["right", "left", "left"], expected=[1, 0, 0],
                ),
                # Variable sequence length
                testing.FeatureExpectationItem(
                    value=["right", "left", "right", "left"], expected=[1, 0, 1, 0],
                ),
                # Empty sequence length
                testing.FeatureExpectationItem(value=[], expected=[],),
            ],
    ...
    def test_getattr(self):
        feature = feature_lib.Sequence(feature_lib.ClassLabel(names=["left", "right"]),)
