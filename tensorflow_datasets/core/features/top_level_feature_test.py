...
class FeaturesManagerTest(testing.TestCase):
    ...
    def test_flatten_nested(self):
        ...
        f = features_lib.FeaturesDict(
            {
                "a": tf.int32,
                "b": {"c": {"d": tf.int32, "e": tf.int32,},},
                "f": features_lib.Sequence(
                    {"g": features_lib.Sequence(tf.int32), "h": tf.int32,}
                ),
            }
        )

        flat1 = f._flatten(
            {"a": "a", "b": {"c": {"d": {"d": 123},},}, "f": {"g": "g",},}
        )
        self.assertEqual(flat1, ["a", {"d": 123}, None, "g", None,])  # 'e'  # h
        self.assertEqual(
            f._nest(flat1),
            {
                "a": "a",
        ...
        f = features_lib.FeaturesDict({"a": tf.int32, "b": {"c": tf.int32,},})
