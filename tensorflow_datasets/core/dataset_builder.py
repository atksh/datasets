  ...
  """
    ...
    """
        ...
        if not self._builder_config and not self.VERSION:
            raise AssertionError(
                ...
                "`VERSION = tfds.core.Version('x.y.z')` to the class.".format(self.name)
            )
    ...
    """
        ...
        if not utils.has_sufficient_disk_space(
            ...
            raise IOError(
                "Not enough disk space. Needed: %s"
            ...
            with utils.temporary_assignment(self, "_data_dir", tmp_data_dir):
                if download_config.try_download_gcs and gcs_utils.is_dataset_on_gcs(
                    ...
                    self._download_and_prepare(
                        dl_manager=dl_manager, download_config=download_config
                    )
                    ...
                    if (
                        download_config.compute_stats == download.ComputeStatsMode.SKIP
    ...
    """
        ...
        logging.info(
            "Constructing tf.data.Dataset for split %s, from %s", split, self._data_dir
        )
    ...
    def _build_single_dataset(
        self, split, shuffle_files, batch_size, decoders, as_supervised, in_memory
    ):
        ...
        dataset_shape_is_fully_defined = dataset_utils.features_shape_is_fully_defined(
            self.info.features
        )
    ...
    def _log_download_done(self):
        msg = (
            "Dataset {name} downloaded and prepared to {data_dir}. "
...
class FileAdapterBuilder(DatasetBuilder):
  ...
  """
    ...
    def _as_dataset(
        self, split=splits_lib.Split.TRAIN, decoders=None, shuffle_files=False
    ):
    ...
    def _slice_split_info_to_instruction_dicts(self, list_sliced_split_info):
        ...
        for sliced_split_info in list_sliced_split_info:
            ...
            filepaths = list(
                sorted(self._build_split_filenames(sliced_split_info.split_info))
            )
            ...
            for filepath, mask_offset in zip(filepaths, mask_offsets):
                instruction_dicts.append(
                    {"filepath": filepath, "mask": mask, "mask_offset": mask_offset,}
                )
  ...
  """
    ...
    def _prepare_split(self, split_generator, max_examples_per_split):
        ...
        if max_examples_per_split is not None:
            logging.warning("Splits capped at %s examples max.", max_examples_per_split)
        ...
        writer = tfrecords_writer.Writer(
            self._example_specs, fpath, hash_salt=split_generator.name
        )
        for key, record in utils.tqdm(
            generator, unit=" examples", total=split_info.num_examples, leave=False
        ):
...
class BeamBasedBuilder(FileAdapterBuilder):
    ...
    def _download_and_prepare(self, dl_manager, download_config):
        ...
        with beam.Pipeline(
            runner=download_config.beam_runner, options=download_config.beam_options,
        ) as pipeline:
    ...
    def _prepare_split(self, split_generator, pipeline):
        ...
        split_info = split_generator.split_info
        output_prefix = naming.filename_prefix_for_split(self.name, split_info.name)
        output_prefix = os.path.join(self._data_dir, output_prefix)
        ...
        _ = (
            pipeline | split_info.name >> _build_pcollection()
