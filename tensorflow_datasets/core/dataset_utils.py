...
def build_dataset(
    instruction_dicts, dataset_from_file_fn, shuffle_files=False, parallel_reads=64
):
  ...
  """
    ...
    # Second case: Use the instructions to read the examples
        ...
        build_ds_from_instruction = functools.partial(
            _build_ds_from_instruction, ds_from_file_fn=dataset_from_file_fn,
        )
...
def _build_ds_from_instruction(instruction, ds_from_file_fn):
  ...
  """
    ...
    mask_ds = _build_mask_ds(
        mask_offset=instruction["mask_offset"], mask=instruction["mask"],
    )
...
def as_numpy(dataset, graph=None):
  ...
  """
    ...
    for ds_el in flat_ds:
        ...
        if not (
            isinstance(ds_el, (tf.Tensor, tf.RaggedTensor))
    ...
    if tf.executing_eagerly():
        ...
        with utils.nogpu_session(graph) as sess:  # Shared session for tf.Tensor
            ...
            np_arrays = sess.run(
                [tensor for tensor in flat_ds if not tf_compat.is_dataset(tensor)]
            )
