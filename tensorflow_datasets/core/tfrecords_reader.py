...
def _get_dataset_from_filename(filename_skip_take, do_skip, do_take):
    ...
    dataset = tf.data.TFRecordDataset(
        filename, buffer_size=_BUFFER_SIZE, num_parallel_reads=1,
    )
    if do_skip:
...
def _read_single_instruction(
    instruction, parse_fn, name, path, name2len, name2shard_lengths, shuffle_files
):
  ...
  """
    ...
    dataset = instruction_ds.interleave(
        functools.partial(_get_dataset_from_filename, do_skip=do_skip, do_take=do_take),
...
class Reader(object):
  ...
  """
    ...
    """
        ...
        name2len = {name: sum(lengths) for name, lengths in name2shard_lengths.items()}
...
class _RelativeInstruction(object):
    ...
    rounding = attr.ib(
        validator=attr.validators.in_(["closest", "pct1_dropremainder"])
    )  # str
    ...
    def __add__(self, other):
        ...
        if self._relative_instructions[0].rounding != other_ris[0].rounding:
            raise AssertionError(
                "It is forbidden to sum ReadInstruction instances "
