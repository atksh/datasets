...
class DummyDataset(tfds.core.GeneratorBasedBuilder):
    ...
    def _info(self):
        return tfds.core.DatasetInfo(
            builder=self, features=tfds.features.FeaturesDict({"value": tf.int64})
        )
    ...
    def values(self, split):
        return [int(v["value"]) for v in tfds.as_numpy(self.as_dataset(split=split))]
...
class SliceToMaskTest(testing.TestCase):
    ...
    def test_slice_to_mask(self):
        ...
        self.assertEqual(s2p(self[10:20]), [False] * 10 + [True] * 10 + [False] * 80)
...
class SplitsOffsetTest(testing.TestCase):
    ...
    def test_compute_mask_offsets(self):
        self.assertEqual(
            splits.compute_mask_offsets([1100, 500, 1100, 110]), [0, 0, 0, 0],
        )
        self.assertEqual(
            splits.compute_mask_offsets([1101, 500, 1100, 110]), [0, 1, 1, 1],
        )
        self.assertEqual(
            splits.compute_mask_offsets([87]), [0],
        )
        self.assertEqual(
            splits.compute_mask_offsets([1101, 501, 1113, 110]), [0, 1, 2, 15],
        )
...
class SplitsIntegrationTest(testing.TestCase):
    ...
    def test_split_merge(self):
        ...
        self.assertEqual(
            list(sorted(all_values)), list(sorted(RANGE_TRAIN + RANGE_TEST)),
        )
    ...
    def test_sub_merge_split(self):
        ...
        train = tfds.Split.TRAIN
        test = tfds.Split.TEST
        split_pos = train.subsplit(tfds.percent[50:]) + test.subsplit(tfds.percent[:10])
        split_neg = train.subsplit(tfds.percent[:50]) + test.subsplit(tfds.percent[10:])
        ...
        # The number of example in each split match the defined subsplit
        self.assertEqual(len(values_pos), len(RANGE_TRAIN) // 2 + len(RANGE_TEST) // 10)
        self.assertEqual(
    ...
    def test_split_invalid(self):
        ...
        with self.assertRaisesWithPredicateMatch(ValueError, "added with itself"):
            split = test.subsplit(tfds.percent[:10]) + test.subsplit(tfds.percent[10:])
            self._builder.values(split=split)

        # Cannot slice a split twice

        with self.assertRaisesWithPredicateMatch(ValueError, "has already been sliced"):
