...
class SuperGlue(tfds.core.GeneratorBasedBuilder):
    ...
    def _info(self):
        features = {
            feature: tfds.features.Text() for feature in self.builder_config.features
        }
        ...
        if self.builder_config.name == "multirc":
            features["idx"] = tfds.features.FeaturesDict(
                {"paragraph": tf.int32, "question": tf.int32, "answer": tf.int32,}
            )
        elif self.builder_config.name == "record":
            features["idx"] = tfds.features.FeaturesDict(
                {"passage": tf.int32, "query": tf.int32,}
            )
        else:
