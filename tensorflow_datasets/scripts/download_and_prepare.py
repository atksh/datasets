...
flags.DEFINE_string(
    "datasets",
    "",
    "Comma separated list of datasets to build, defaults to all" "registered builders.",
)
flags.DEFINE_string(
    "exclude_datasets",
    "",
    "Comma separated list of datasets to exclude," "(no download, no prepare).",
)
...
flags.DEFINE_string(
    "checksums_dir",
    None,
    "For external datasets, specify the location of the " "dataset checksums.",
)
...
# Development flags
flags.DEFINE_boolean(
    "register_checksums", False, "If True, store size and checksum of downloaded files."
)

# Debug flags
flags.DEFINE_boolean(
    "debug", False, "If True, will drop into debugger after data generation"
)
flags.DEFINE_boolean(
    "debug_start", False, "If True, will drop into debugger on startup"
)
flags.DEFINE_boolean(
    "sleep_start", False, "If True, will sleep on startup; useful for ssh"
)
...
def main(_):
    ...
    datasets_to_build = set(
        FLAGS.datasets and FLAGS.datasets.split(",") or tfds.list_builders()
    )
    ...
    logging.info(
        "Running download_and_prepare for datasets:\n%s", "\n".join(datasets_to_build)
    )
