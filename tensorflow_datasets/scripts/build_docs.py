...
flags.DEFINE_string("output_dir", "/tmp/datasets_api", "Where to output the docs")
flags.DEFINE_string(
    "code_url_prefix",
...
def build_api_docs(output_dir, code_url_prefix, search_hints, site_path):
    ...
    for before, after in MOVES:
        ...
        new_redirects.append(
            {
                "from": os.path.join(
                    "/datasets/api_docs/python/", os.path.splitext(before)[0]
                ),
                "to": os.path.join(
                    "/datasets/api_docs/python/", os.path.splitext(after)[0]
                ),
            }
        )
...
def main(unused_argv):
    build_api_docs(
        FLAGS.output_dir, FLAGS.code_url_prefix, FLAGS.search_hints, FLAGS.site_path
    )
